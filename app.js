...
async function loadDashboard() {
    try {
        ...
        const { count: monthly } = await supabase.from('documents').select('*',{count:'exact', head:true}).gte('created_at', firstDayOfMonth.toISOString());
        ...
        const { data: recent } = await supabase.from('documents').select('*, subjects(name)').order('created_at',{ascending:false}).limit(10);
...
window.loadSemesters = async function(branchId, branchName) {
    const { data: semesters } = await supabase.from('semesters').select('*').eq('branch_id',branchId).order('name');
    const branchesList = document.getElementById('branches-list');
    branchesList.innerHTML = `
        <button onclick="loadBranches()" class="back-btn">← Back to Branches</button>
        <h3>${branchName} - Semesters</h3>
        ${semesters && semesters.length > 0 ? semesters.map(sem => `
        <div class="semester-card" onclick="loadSections('${sem.id}','${sem.name}')">
            <h4>📖 ${sem.name}</h4>
            <p>${sem.description||''}</p>
        </div>`).join('') : '<p>No semesters found</p>'}
    `;
}
window.loadSections = async function(semesterId, semesterName){
    const {data: sections} = await supabase.from('sections').select('*').eq('semester_id',semesterId).order('name');
    const branchesList = document.getElementById('branches-list');
    branchesList.innerHTML = `
        ...
        ${sections && sections.length > 0 ? sections.map(sec => `
            <div class="section-card" onclick="loadSubjects('${sec.id}','${sec.name}')">
                <h4>📂 ${sec.name}</h4>
                <p>${sec.description||''}</p>
            </div>`).join('') : '<p>No sections found</p>'}
    `;
}
window.loadSubjects = async function(sectionId, sectionName){
    const {data: subjects} = await supabase.from('subjects').select('*').eq('section_id',sectionId).order('name');
    const branchesList = document.getElementById('branches-list');
    branchesList.innerHTML = `
        <button onclick="loadBranches()" class="back-btn">← Back</button>
        ...
        ${subjects && subjects.length>0 ? subjects.map(subj => `
            ...
            </div>`).join(''): '<p>No subjects found</p>'}
    `;
}
window.loadDocuments = async function(subjectId, subjectName){
    const {data: documents} = await supabase.from('documents').select('*').eq('subject_id',subjectId).order('created_at',{ascending:false});
    const branchesList = document.getElementById('branches-list');
    branchesList.innerHTML = `
        <button onclick="loadBranches()" class="back-btn">← Back</button>
        ...
        </div>`).join('') : '<p>No documents found</p>'}
    `;
}
...
async function loadFilters(){
    ...
    branchFilter.innerHTML = '<option value="">All Branches</option>'+ (branches||[]).map(b => `<option value="${b.id}">${b.name}</option>`).join('');
    const {data: subjects} = await supabase.from('subjects').select('*').order('name');
    const subjectFilter = document.getElementById('subject-filter');
    subjectFilter.innerHTML = '<option value="">All Subjects</option>' + (subjects||[]).map(s => `<option value="${s.id}">${s.name}</option>`).join('');
};
window.performSearch = async function(){
    const searchTerm = document.getElementById('search-input').value.toLowerCase();
    const subjectFilter = document.getElementById('subject-filter').value;
    let query = supabase.from('documents').select('*, subjects(name)');
    if(subjectFilter){
    ...
    if(searchTerm){
        filtered = filtered.filter(doc => doc.title.toLowerCase().includes(searchTerm) || (doc.description && doc.description.toLowerCase().includes(searchTerm)) || (doc.subjects?.name && doc.subjects.name.toLowerCase().includes(searchTerm)));
    }
...
async function loadUploadOptions(){
    ...
    const {data: sections} = await supabase.from('sections').select('*, semesters(name, branches(name))').order('name');
    const sectionSelect = document.getElementById('upload-section-select');
    sectionSelect.innerHTML = '<option value="">Choose section...</option>'+ (sections||[]).map(s => `<option value="${s.id}">${s.semesters?.branches?.name || ''} - ${s.semesters?.name || ''} - ${s.name}</option>`).join('');
    sectionSelect.onchange = async (e) => {
        const sectionId = e.target.value;
        if(sectionId){
            const {data: subjects} = await supabase.from('subjects').select('*').eq('section_id',sectionId).order('name');
            const subjectSelect = document.getElementById('upload-subject-select');
            subjectSelect.innerHTML = '<option value="">Choose subject...</option>' + (subjects||[]).map(s => `<option value="${s.id}">${s.name}</option>`).join('');
        }
    };
}
window.uploadFile = async function(){
    ...
    try{
        ...
        const {data: urlData} = supabase.storage.from('university-notes-files').getPublicUrl(fileName);
...
async function loadManageOptions(){
    ...
    semesterBranchSelect.innerHTML = '<option value="">Select Branch</option>'+ (branches||[]).map(b => `<option value="${b.id}">${b.name}</option>`).join('');
    semesterBranchSelect.onchange = async (e) => {
        const branchId = e.target.value;
        if(branchId){
            const {data: semesters} = await supabase.from('semesters').select('*').eq('branch_id',branchId).order('name');
            const sectionSemesterSelect = document.getElementById('section-semester-select');
            sectionSemesterSelect.innerHTML = '<option value="">Select Semester</option>'+ (semesters||[]).map(s => `<option value="${s.id}">${s.name}</option>`).join('');
        }
    };
    document.getElementById('section-semester-select').onchange = async (e) => {
        ...
        if(semesterId){
            ...
            subjectSectionSelect.innerHTML = '<option value="">Select Section</option>'+ (sections||[]).map(s => `<option value="${s.id}">${s.name}</option>`).join('');
        }
    };
}
window.createBranch = async function(){
    ...
    try{
        const {error} = await supabase.from('branches').insert([{name, description}]);
        if(error) throw error;
...
window.createSemester = async function(){
    ...
    try{
        const {error} = await supabase.from('semesters').insert([{branch_id: branchId, name, description}]);
        if(error) throw error;
...
window.createSection = async function(){
    ...
    try{
        const {error} = await supabase.from('sections').insert([{semester_id: semesterId, name, description}]);
        if(error) throw error;
...
window.createSubject = async function(){
    ...
    try{
        const {error} = await supabase.from('subjects').insert([{section_id: sectionId, name, description}]);
        if(error) throw error;
